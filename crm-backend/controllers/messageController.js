...
const getRandomStatus = () => (Math.random() < 0.9 ? 'SENT' : 'FAILED');
...
const publishToQueue = async (queueName, message) => {
    try {
        ...
        throw new Error('Queue publishing failed');
    }
};
    ...
    try {
        ...
        for (const customer of customers) {
            ...
            const [result] = await pool.query(
                ...
                [campaignId, customer.id, personalizedMessage, 'SENT']
            );

            const logId = result.insertId;

            // Publish the logId and message details to the queue
            await publishToQueue('message_logs', { logId, personalizedMessage });
...
// Controller to fetch message logs for a specific campaign
exports.getMessageLogs = async (req, res) => {
    ...
    try {
        ...
        const [logs] = await pool.query('SELECT * FROM message_logs WHERE campaignId = ?', [
            campaignId,
        ]);
